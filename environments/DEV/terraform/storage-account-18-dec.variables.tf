variable "storage-account-18-dec__module_source" {
  default = "git::https://github.com/Mia-Platform-Experiments/tf-azure-storage.git"
}

variable "storage-account-18-dec_service_name" {
  ...
  validation {
    condition     = can(regex("^[a-z0-9]{3,24}$", var.storage-account-18-dec_service_name))
    error_message = "Service name must be 3-24 lowercase alphanumeric characters only (no hyphens or special characters)."
  }
}

variable "storage-account-18-dec_resource_group_name" {
  description = "The name of the existing Resource Group in Azure."
  type        = string
  default     = "terraform-demo"
}

variable "storage-account-18-dec_location" {
  description = "The Azure region to deploy to."
  type        = string
  default     = "North Europe"
}

variable "storage-account-18-dec_performance_profile" {
  ...
  validation {
    condition     = contains(["sandbox", "development", "production"], var.storage-account-18-dec_performance_profile)
    error_message = "Performance profile must be one of: sandbox, development, production."
  }
}

variable "storage-account-18-dec_container_names" {
...
variable "storage-account-18-dec_access_tier" {
